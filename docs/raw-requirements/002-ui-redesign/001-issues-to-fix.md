...
- **Fix:** Add utility classes in `globals.css`:
  ```css
  @layer components {
    .text-page-title { @apply text-xl font-extrabold text-white; }
    .text-eyebrow { font-size: 0.6rem; letter-spacing: 0.15em; text-transform: uppercase; color: var(--text-secondary); }
    .text-card-title { font-size: 0.6875rem; font-weight: 700; color: white; }
    .text-status-chip { font-size: 0.5rem; font-weight: 700; text-transform: uppercase; }
  }
  ```
